...
function findNumberOfWorkout(){
	console.log("find number of workouts has been accessed")
			let xhr = new XMLHttpRequest();
			xhr.open('GET', 'api/index')
			xhr.onreadystatechange = function(){
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
				let workouts = workouts.length;
				
					let dataDiv = document.getElementById('workoutData');
					dataDiv.textContent = 'This is the total number of workouts logged, great job!';
					dataDiv.appendChild(workouts);
